...
object P011 {
  ...
  def getHorizontalNums(i: Int, j: Int, n: Int): Array[Int] = (i, j) match {
    case (i, _) if i >= grid.length => Array.empty
    case (_, j) if j > grid.length - n => Array.empty
    case _ => grid(i).drop(j).take(n)
  }

  def getVerticalNums(i: Int, j: Int, n: Int): Array[Int] = (i, j) match {
    case (i, _) if i > grid.length - n=> Array.empty
    case (_, j) if j >= grid.length => Array.empty
    case _ => grid.slice(i, i + n).map { _(j) }
  }

  def getDiagonalNums1(i: Int, j: Int, n: Int): Array[Int] = (i, j) match {
    case (i, _) if i > grid.length - n => Array.empty
    case (i, _) if i >= grid.length => Array.empty
    case (_, j) if j >= grid.length => Array.empty
    case (_, j) if j > grid.length - n => Array.empty
    case _ => (0 until n).map { k => grid(i + k)(j + k) }.toArray
  }

  def getDiagonalNums2(i: Int, j: Int, n: Int): Array[Int] = (i, j) match {
    case (i, _) if i > grid.length - n => Array.empty
    case (i, _) if i >= grid.length => Array.empty
    case (_, j) if j >= grid.length => Array.empty
    case (_, j) if j < n - 1 => Array.empty
    case _ => (0 until n).map { k => grid(i + k)(j - k) }.toArray
  }

  def solve(n: Int): Long = {
    (0 until grid.length).flatMap { i =>
      (0 until grid.length).map { j =>
        (i, j)
      }
    }.map { case (i, j) =>
      val h = getHorizontalNums(i, j, n).product
      val v = getVerticalNums(i, j, n).product
      val d1 = getDiagonalNums1(i, j, n).product
      val d2 = getDiagonalNums2(i, j, n).product

      h.max(v).max(d1).max(d2)
    }.max
  }
}
